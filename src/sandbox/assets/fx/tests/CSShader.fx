...
void __spawn_op__(uint nPart)
{
	...
	for (int i = 0; i < nGroups; ++i)
	{
		...
		RequestId = uavSpawnDispatchArguments[0];
		uavSpawnDispatchArguments[0] = RequestId + 1u;
		uavCeatetionRequests[RequestId] = uint2(min(nPart, 64u), 0u);
		nPart = nPart - 64u;
	}
}
...
void CSParticlesSpawnRoutine(uint3 Gid: SV_GroupID, uint GI: SV_GroupIndex, uint3 GTid: SV_GroupThreadID, uint3 DTid: SV_DispatchThreadID)
{
	...
	uavSpawnDispatchArguments[3] = (uint)nPartAddFloat;//asuint(nPartAddFloat - nPartAdd);
	__spawn_op__((uint)nPartAdd);
}
...
void CSParticlesResetRoutine(uint3 Gid: SV_GroupID, uint GI: SV_GroupIndex, uint3 GTid: SV_GroupThreadID, uint3 DTid: SV_DispatchThreadID)
{
	...
	Particle.size = float3(0.f, 0.f, 0.f);
...
AppendStructuredBuffer<PartInstance> uavPrerendered0: register(u5);
...
AppendStructuredBuffer<PointInstance> uavPrerendered1: register(u6);
