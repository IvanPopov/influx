...
[
    ...
    class Node extends LGraphNodeAST {
        ...
        evaluate(context: Context, program: ProgramScope): IExprInstruction {
            ...
            const args= [...Array(count).keys()].map(i => {
                const input = this.getInputNode(i) as IGraphASTNode;
                ...
                return input.run(context, program, this.link(0)) as IExprInstruction;
            });
    ...
    LiteGraph.registerNodeType(`constructors/${typeName}`, Node);
});
