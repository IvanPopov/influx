        ...
        try {
            ...
            while (!breakProcessing) {
                ...
                if (isDef(operation)) {
                    ...
                    // const synchronizingTokens = [';', '{', '}', '(', ')', '[', ']'];
                    
                    // do {
                    //     const node = tree.removeNode();
                    //     // console.log(node);
                    //     lastToken = node.value;
                    //     stack.pop();
                    //     console.warn(`stack.pop(${lastToken})`);
                    // } while (synchronizingTokens.indexOf(lastToken) === -1 && tree.getNodes().length);
                }
            }
