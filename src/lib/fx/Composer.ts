...
import { IAFXBlender } from "../idl/IAFXBlender";
import { IMap } from "../idl/IMap";
import { IAFXComponentBlend } from "../idl/IAFXComponentBlend";
import { IAFXComposer } from "../idl/IAFXComposer";
import { EPrimitiveTypes } from "../idl/EPrimitiveTypes";
import { isNull, isDef } from "../common";
import { IAFXComponent } from "../idl/IAFXComponent";
import { IEffect } from "../idl/IEffect";
import { IParseTree } from "../idl/parser/IParser";
...
export class Composer implements IAFXComposer {
	private _pTechniqueToBlendMap: IMap<IAFXComponentBlend> = null;
	private _pTechniqueToOwnBlendMap: IMap<IAFXComponentBlend> = null;
	private _pTechniqueLastGlobalBlendMap: IMap<IAFXComponentBlend> = null;
	private _pTechniqueNeedUpdateMap: IMap<boolean> = null;

	private _pEffectResourceToComponentBlendMap: IMap<IAFXComponentBlend> = null;
	private _pBlender: IAFXBlender = null;

	private _pGlobalEffectResorceIdStack: number[] = null;
	// private _pGlobalEffectResorceShiftStack: number[] = null;
	...
	constructor(pEngine: IEngine) {
		...
		this._pTechniqueToBlendMap = <IMap<IAFXComponentBlend>>{};
		this._pTechniqueToOwnBlendMap = <IMap<IAFXComponentBlend>>{};
		this._pTechniqueLastGlobalBlendMap = <IMap<IAFXComponentBlend>>{};
		this._pTechniqueNeedUpdateMap = <IMap<boolean>>{};

		this._pEffectResourceToComponentBlendMap = <IMap<IAFXComponentBlend>>{};
	...
	getComponentCountForEffect(pEffectResource: IEffect): number {
		var id: number = pEffectResource.getResourceHandle();
	...
	copyTechniqueOwnComponentBlend(pFrom: IRenderTechnique, pTo: IRenderTechnique): void {
		var iFromId: number = pFrom.guid;
		var iToId: number = pTo.guid;
	...
	getMinShiftForOwnTechniqueBlend(pRenderTechnique: IRenderTechnique): number {
		var id: number = pRenderTechnique.guid;
	...
	prepareTechniqueBlend(pRenderTechnique: IRenderTechnique): boolean {
		...
		if (isTechniqueUpdate || isUpdateGlobalBlend) {
			var iEffect: number = pRenderTechnique.getMethod().getEffect().getResourceHandle();
	...
	_loadEffectFromSyntaxTree(pTree: IParseTree, sFileName: string): boolean {		if (config.AFX_ENABLE_TEXT_EFFECTS) {
	...
	_calcRenderID(pSceneObject: ISceneObject, pRenderable: IRenderableObject, bCreateIfNotExists: boolean = false): number {
		//assume, that less than 1024 draw calls may be & less than 1024 scene object will be rendered.
		//beacause only 1024

		var iSceneObjectGuid: number = !isDefAndNotNull(pSceneObject) ? 0 : pSceneObject.guid;
		var iRenderableObjectGuid: number = !isDefAndNotNull(pRenderable) ? MAX_UINT32 : pRenderable.guid;
		...
		var pRidByRenderable: IMap<number> = pRidTable[iSceneObjectGuid];
