...
export function createSyncFXSLDocument(textDocument: ITextDocument, flags?: number, document?: ISLDocument): ISLDocument;
export function createSyncFXSLDocument(slastDocument: ISLASTDocument, document?: ISLDocument): ISLDocument;
export function createSyncFXSLDocument(param1: ISLASTDocument | ITextDocument, param2?: number | ISLDocument, param3?: ISLDocument): ISLDocument {
    let textDocument: ITextDocument;
    let slastDocument: ISLASTDocument;
    let slDocument: ISLDocument;
    ...
    const analyzer = new FxAnalyzer;
    return analyzer.parseSync(slastDocument, { document: slDocument });
}