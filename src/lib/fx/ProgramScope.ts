...
export class ProgramScope {
	...
	_addVariable(pVariable: IAFXVariableDeclInstruction, iScope: number = this._iCurrentScope): boolean {
		if (isNull(iScope)) {
			return false;
		}
	...
	_addType(pType: IAFXTypeDeclInstruction, iScope: number = this._iCurrentScope): boolean {
		if (isNull(iScope)) {
			return false;
		}
	...
	_addFunction(pFunction: IAFXFunctionDeclInstruction, iScope: number = ProgramScope.GLOBAL_SCOPE): boolean {
		if (isNull(iScope)) {
			return false;
		}
	...
	_hasVariable(sVariableName: string, iScope: number = this._iCurrentScope): boolean {
		if (isNull(iScope)) {
			return false;
		}
	...
	_hasType(sTypeName: string, iScope: number = this._iCurrentScope): boolean {
		if (isNull(iScope)) {
			return false;
		}
	...
	_hasFunction(sFuncName: string, pArgumentTypes: IAFXTypedInstruction[], iScope: number = ProgramScope.GLOBAL_SCOPE): boolean {
		if (isNull(iScope)) {
			return false;
		}
