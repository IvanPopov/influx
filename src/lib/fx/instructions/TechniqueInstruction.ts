...
import { IAFXTechniqueInstruction, IAFXVariableDeclInstruction, IAFXPassInstruction, IAFXImportedTechniqueInfo, EAFXInstructionTypes, EFunctionType } from "../../idl/IAFXInstruction";
...
export class TechniqueInstruction extends DeclInstruction implements IAFXTechniqueInstruction {
	...
	private generateListOfSharedVariables(): void {
		...
		for (var i: number = 0; i < this._pPassList.length; i++) {
			var pSharedV: IMap<IAFXVariableDeclInstruction> = this._pPassList[i]._getSharedVariableMapV();
			var pSharedP: IMap<IAFXVariableDeclInstruction> = this._pPassList[i]._getSharedVariableMapP();
	...
	private generateFullListOfComponent(): void {
		...
		for (var i: number = 0; i < this._pImportedTechniqueList.length; i++) {
			...
			var pAddComponentShiftList: number[] = pTechnique._getFullComponentShiftList();
