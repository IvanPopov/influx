...
import { createBaseEmitter } from "./BaseEmitter";
...
export function createCodeEmitter(ctx = createBaseEmitter()) {
    const {
        depth,
        begin,
        end,
    ...
    const emitAttribute = (src: IVariableDeclInstruction) =>
        (kw('attribute'), emitVariableDecl(src, sname.attr), add(';'), nl());
    const emitVarying = (src: IVariableDeclInstruction) =>
...
export type ICodeEmitter = ReturnType<typeof createCodeEmitter>;
